...
Map voltResponse2 = {'zero': 0, 'one': 1, 'two': 2};
Map tempResponse2 = {'zero': 0, 'one': 1, 'two': 2};
Map socResponse2 = {'zero': 0, 'one': 1, 'two': 2};
Map sohResponse2 = {'zero': 0, 'one': 1, 'two': 2};
double volt2 = 0.0000;
double temp2 = 0.0000;
double soc2 = 0.0000;
double soh2 = 0.0000;
...
class cell_2 extends StatefulWidget {
  const cell_2({super.key});

  @override
  State<cell_2> createState() => _cell_2State();
}

class _cell_2State extends State<cell_2> {

  Future Voltage () async{
    http.Response response;
    response = await http.get(Uri.parse("https://api.thingspeak.com/channels/2295970/fields/2.json?results=1"));
    if(response.statusCode == 200)
    {
      setState(() {
        //stringResponse = response.body;
        voltResponse2 = json.decode(response.body);
        volt2 = double.parse(voltResponse2['feeds'][0]["field2"]);
        //print(volt2);
      });
    }
  }
  ...
  Future Temperature () async{
    http.Response response;
    response = await http.get(Uri.parse("https://api.thingspeak.com/channels/2310566/fields/2.json?results=1"));
    if(response.statusCode == 200)
    {
      setState(() {
        //stringResponse = response.body;
        tempResponse2 = json.decode(response.body);
        temp2 = double.parse(tempResponse2['feeds'][0]["field2"]);
       // print(temp2);
      });
    }
  }
  ...
  Future Soc () async{
    http.Response response;
    response = await http.get(Uri.parse("https://api.thingspeak.com/channels/2295973/fields/2.json?results=1"));
    if(response.statusCode == 200)
    {
      setState(() {
        //stringResponse = response.body;
        socResponse2 = json.decode(response.body);
        soc2 = double.parse(socResponse2['feeds'][0]["field2"]);
       // print(soc2);
      });
    }
  }
  ...
  Future Soh () async{
    http.Response response;
    response = await http.get(Uri.parse("https://api.thingspeak.com/channels/2310585/fields/2.json?results=1"));
    if(response.statusCode == 200)
    {
      setState(() {
        //stringResponse = response.body;
        sohResponse2 = json.decode(response.body);
        soh2 = double.parse(sohResponse2['feeds'][0]["field2"]);
        // print(soh2);
      });
    }
  }
  ...
  Future gauge() async{
    key.currentState!.updateSpeed(temp2);
    speedNotifier.value = temp2;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: NavBar(),
      appBar: AppBar(title: Text("Status of Cell 2"),
          ...
          InkWell(
            ...
            child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(20),color: Colors.green,),
              child: Column(
                ...
                children: [
                  Icon(Icons.battery_charging_full_sharp,size: 150,color: volt2<2.2 ? Colors.red : Color(0xff006400),),
                  Text(volt2==0 ? 'Loading\nVoltage' : volt2.toStringAsFixed(3)+' V\nVoltage',style: TextStyle(color: Colors.white,fontSize: 26, fontWeight: FontWeight.bold),)
                ],),
            ),
          ),
          InkWell(
                ...
                children: [
                  Center(
                    child: AnimatedBatteryGauge(
                      ...
                      value: soc2,
                      size: Size(150, 70),
                      borderColor: soc2<30 ? CupertinoColors.destructiveRed : CupertinoColors.activeGreen,
                      valueColor: soc2<30 ? CupertinoColors.destructiveRed : CupertinoColors.activeGreen,
                      hasText: false,
                      mode: BatteryGaugePaintMode.grid,
                    ),
                  ),
                  Text(soc2==0 ? '\nLoading\nState-of-Charge' :'\n'+soc2.toStringAsFixed(3)+' %\nState-of-Charge',style: TextStyle(color: Colors.white,fontSize: 23,fontWeight: FontWeight.bold,),textAlign: TextAlign.center,)
                ],),
            ),
          ),
          InkWell(
            ...
            child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(20),color: Colors.green,),
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.battery_saver_sharp,size: 150,color: soh2<50 ? Colors.red : Color(0xff006400),),
                  Text(soh2==0 ? 'Loading\nState-of-Health' : soh2.toStringAsFixed(3)+'%\nState-of-Health',style: TextStyle(color: Colors.white,fontSize: 23,fontWeight: FontWeight.bold,),textAlign: TextAlign.center,)
                ],),
            ),
          ),
        ],
        ...
        children:[
          SizedBox(
            ...
            child: FloatingActionButton(
              ...
              child: Builder(
                builder: (context){
                  if(volt2==0 && temp2==0 && soc2==0 && soh2==0)
                  {
                  ...
                  else if(temp2>=45)
                  {
                    ...
                    return Text('Comment: SOH of cell 2 is too Low, replace Cell 2.',style: TextStyle(color: Colors.black,fontSize: 20),);
                  }
                  else if(soc2<30){
